...
function Cart() {
    const ArrayCardsinCart = useRef()
    const dispatch = useDispatch()
    const [arrCart, setArrCart] = useState([])
    const [renderCard, setRenderCard] = useState(null)
    const [AmountAllCards, setAmountAllCards] = useState(0)
    const cartValuePrice = useSelector(state => state.price.cartPrice)
    useEffect(() => {
        axios.get('https://61e1476a63f8fc0017618b27.mockapi.io/cart_buy_to_pizzas').then(res => {
            setArrCart(res.data)
            setAmountAllCards(res.data.length)
        })
    }, [])
    ...
    const summaPrice = summ(cartValuePrice)
    const obj = {
        price: summaPrice,
        items: AmountAllCards
    }
        ...
        <div className="content">
            <div className="container container--cart">
                <div className="cart">
                    ...
                    <div className="content__items" ref={ArrayCardsinCart}>
                        {
                            arrCart.map((obj, index) => (
                                <CartItem renderCard={renderCard} setRenderCard={setRenderCard} key={index} {...obj} />
                            ))
                        }
                    </div>
                    <div className="cart__bottom">
                        <div className="cart__bottom-details">
                            <span> Всего пицц: <b>{AmountAllCards} шт.</b> </span>
                            <span> Сумма заказа: <b>{summaPrice} ₽</b> </span>
                        </div>
